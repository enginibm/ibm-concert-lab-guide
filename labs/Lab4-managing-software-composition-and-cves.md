...
Here is the structure that you will have after the clone of the repositories:
  ...
  <br><img src="../images/tree.png" alt="drawing" width="400"/>

In the folder SBOMs-ingestion you have:

1. **scripts/generate-sboms.sh**: 
   
A shell script that take as parameter the application for which we want to generate SBOMs.  
This script will:
...
- Generate the code scan SBOM with the concert toolkit. Concert toolkit use Cyclone DX.
- Generate the image scan SBOM with the concert toolkit. Concert toolkit use Syft.
- Generate the image CVEs using a Trivy
...
This script use Concert APIs to upload the SBOMs in IBM Concert

1. **app-common-variables.variables**

The environment variables used by the scripts whatever the application.

4. **hr-app.variables**

The environment variables with valued specific to hr-app application.

5. **summarization-svc.variables**

The environment variables with valued specific to summarization-svc application.
...
- Home page - Software composition Lens
- Arena view
